...
export default class LeafpressPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      callback: () => this.commandHandlers.initialize(),
    });

    this.addCommand({
      id: "leafpress-build",
      name: "Build Site",
      callback: () => this.commandHandlers.build(),
    });

    this.addCommand({
      id: "leafpress-preview",
      name: "Preview Site",
      callback: () => this.commandHandlers.preview(),
    });

    this.addCommand({
      id: "leafpress-deploy",
      name: "Deploy",
      callback: () => this.commandHandlers.deploy(),
    });
    ...
    this.registerView(
      VIEW_TYPE_LEAFPRESS,
      (leaf) => new LeafpressPanel(leaf, this.app.vault)
    );
