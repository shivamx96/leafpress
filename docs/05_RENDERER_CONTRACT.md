...
The CLI honors `build`; the renderer accepts and validates it for shape parity
but performs no I/O. The `deploy` object is a deprecated compatibility field.
Both interfaces accept its shape, but neither performs deployment or reads its
values.
