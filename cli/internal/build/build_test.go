...
func TestBuildSharesOneContentAddressedClientScript(t *testing.T) {
	...
	if strings.Contains(client, "lp-share-actions") {
	...
	clientPath, client = readClientScript(t, siteDir)
	if clientPath == oldPath || !strings.Contains(client, "lp-share-actions") {
	...
	if !strings.Contains(string(noteHTML), `class="lp-share-actions"`) {
		t.Fatal("sharing-enabled build is missing the page actions")
	}
